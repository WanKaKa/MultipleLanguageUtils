import os
import re
from multiple_language.kevin_utils import java_string_file_name_list, filter_string_key_regular
from multiple_language.kevin_utils import get_log_path, print_log, analysis_equal_string
...
def delete_android_values_string(project_res_dir, input_strings, button1):
    ...
    log_file = open(get_log_path() + "delete_string.log", mode='w', encoding='utf-8')

    button1['text'] = "正在删除中，请勿重复点击"
    delete_string_key_list = analysis_equal_string(input_strings, log_file)
            ...
            for res_dir in dirs:
                if os.path.exists(project_res_dir + "\\" + res_dir):
                    if res_dir != "values" and "values" in res_dir:
                        print_log(log_file, "*" * 50)
                        print_log(log_file, res_dir)
