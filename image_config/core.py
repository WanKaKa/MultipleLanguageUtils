...
def run(service_image_path, work_image_path, log_file=None, callback=None):
    ...
    for file in natsorted(os.listdir(work_image_path + "/skin_original/"), alg=ns.PATH):
        if os.path.isdir(work_image_path + "/skin_original/" + file) and file not in FILTER_DIR_LIST:
...
def modify_image(service_image_path, work_image_path, image_type, log_file=None):
    ...
    image_dir_path = work_image_path + "/skin_original/" + image_type + "/"
    image_file_list = []
...
def modify_image_name(work_image_path, image_type, start_index, image_file_list, log_file=None):
    ...
    while index >= 0:
        ...
        old_file = work_image_path + "/skin_original/" + image_type + "/" + image_file_list[index]
        new_file = work_image_path + "/skin_original/" + image_type + "/" + image_type + "_" + int2str(new_index)
...
def get_all_image(work_image_path):
    temp_list = []
    for name1 in natsorted(os.listdir(work_image_path + "/skin_original"), alg=ns.PATH):
        file_path1 = work_image_path + "/skin_original/" + name1
        if os.path.isdir(file_path1) and name1 not in FILTER_DIR_LIST:
            for name2 in natsorted(os.listdir(file_path1), alg=ns.PATH):
                file_path2 = work_image_path + "/skin_original/" + name1 + "/" + name2
